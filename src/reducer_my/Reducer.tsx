...
export const setSettingErrorAC = (error: boolean) =>
  ({ type: SET_SETTING_ERROR, error } as const);
export const disableSettingButtonAC = (disable: boolean) =>
  ({ type: DISABLE_SETTING_BUTTON, disable } as const);
...
export const reducer = (state: statePT = initialState, action: actionPT): statePT => {
  switch (action.type) {
    case ADD:
      return state.presentValue !== state.maxValue
        ? { ...state, presentValue: state.presentValue + ONE }
        : state;
    case RES:
